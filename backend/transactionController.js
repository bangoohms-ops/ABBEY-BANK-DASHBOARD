...
// --- 📧 1. REAL EMAIL GATEWAY ENGINE (GMAIL SMTP) ---
// Note: If you want to use Gmail, replace your transporter setup with this structure.
// Remember to use a 16-character Google App Password, NOT your regular password!
const emailTransporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "bangoohms@gmail.com", // <-- Put your default sender address here
    pass: "owuofapwkfkcvsvs", // <-- Paste your 16-character Google App Password here
  },
});

const sendTransactionAlerts = async (
  sender,
  receiver,
  amount,
  reference,
  ...
  try {
    ...
    if (receiver.email) {
      try {
        ...
        console.log(
          `[EMAIL DISPATCH SUCCESS] -> Drop pushed cleanly to: ${receiver.email}`,
        );
      } catch (emailErr) {
        console.error(
          "❌ Outbound Email Network Pipeline Exception:",
          emailErr.message,
        );
      }
    }
    ...
    if (rawPhone) {
      try {
        ...
        // Dynamically swap the message body text structure if it's an Airtime top-up transaction
        let smsText = `Akoka Bank Credit Alert!\nAmt: ${localCurrencyAmount} CR\nFrom: ${sender.username}\nRef: ${reference.substring(0, 16)}`;

        if (metadata.utilityType === "Airtime") {
          smsText = `Akoka Bank TopUp!\nSuccessful: ₦${Number(amount).toLocaleString()} ${metadata.airtimeNetwork || "Airtime"} sent to ${metadata.airtimePhone}.`;
        }

        await axios.post("https://api.ng.termii.com/api/sms/send", {
          to: formattedPhone,
          from: "Sandbox", // <-- Change this to the global unverified sandbox route
          sms: smsText,
          type: "plain",
          channel: "generic",
          api_key:
            "TLrjyuXaYPmhPbHRUCwucCKjcwWXptyQylhnexNNGuPCgpqYEWGMenwTDlvGOA",
        });
        console.log(
          `[SMS SEND SUCCESS] -> Outbound payload delivered to cellular link: ${formattedPhone}`,
        );
      } catch (smsErr) {
        console.error(
          "❌ SMS Outbound Service Pipeline Exception:",
...
const transferFunds = async (req, res) => {
  ...
  // Destructure standard fields + utility context variables pushed down from frontend App.js
  const {
    senderId,
    receiverAccount,
    amount,
  ...
  if (!senderId || !receiverAccount || !amount || !pin) {
    return res
      ...
      .json({
        success: false,
        message: "Missing required operational transaction fields",
      });
  }
  ...
  try {
    ...
    const queryField =
      typeof senderId === "string" && senderId.includes("@") ? "email" : "id";
    const userRes = await client.query(
      `SELECT id, username, transaction_pin, balance, email, phone_number FROM users WHERE ${queryField} = $1`,
      [senderId],
    );
    ...
    if (!sender) {
      ...
      return res
        .status(444)
        .json({ success: false, message: "Sender record missing." });
    }
    ...
    if (!isPinValid && String(sender.transaction_pin) !== String(pin)) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Invalid Transaction Authorization PIN",
        });
    }
    ...
    if (senderBalanceNum < transferAmountNum) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Insufficient funds inside the ledger cache registry",
        });
    }

    // 4. LOCATE VALID DESTINATION RECEIVER REGISTER
    const receiverRes = await client.query(
      "SELECT id, username, balance, email, phone_number FROM users WHERE account_number = $1",
      [receiverAccount],
    );
    if (receiverRes.rowCount === 0) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Targeted recipient account registry index missing",
        });
    }
    ...
    if (sender.id === receiver.id) {
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Self-transfers to your own destination profile are restricted",
        });
    }
    ...
    // Fire notifications safely out-of-band with utility metadata context
    ...
    client.release();
    console.error(
      "❌ Exception Intercepted inside Ledger Stream:",
      error.message,
    );
